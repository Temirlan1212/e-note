...
export default async function handler(req: NextApiRequest, res: NextApiResponse<any | null>) {
  const { saleOrderId, partnerId } = req.query;

  if (req.method !== "POST" || saleOrderId == null || partnerId == null) {
  ...
  const response = await fetch(
    process.env.BACKEND_API_URL + `/ws/file/upload/?saleOrderId=${saleOrderId}&partnerId=${partnerId}`,
    {
      method: "POST",
      headers: {
